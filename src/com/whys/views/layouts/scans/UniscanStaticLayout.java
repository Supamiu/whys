...
public class UniscanStaticLayout extends ScanLayout implements ScanLayoutInterface{

	private UniscanStatic uni = new UniscanStatic();
	Panel panel = new Panel("Scan statique");
	
	public UniscanStaticLayout(){
		panel.setSizeFull();
		...
		uni.addScanEndListener(new ScanEndListener() {			
			...
			public void onScanTerminated() {
				try {
					scan = neo.getScanOfSite(site, UniscanStatic.NAME);
		...
		try{
			this.scan = neo.getScanOfSite(site, UniscanStatic.NAME);
		}catch(ScanNotFoundException e){
		}
		if(scan != null){
			if(scan.isFinished()){
	...
	public Layout resultLayout(){
		...
		pl.setMargin(true);
		pl.setSpacing(true);
		for(Flaw f : neo.getScanFlaws(scan)){
			Label temp = new Label(f.name+" : "+f.data);
	...
	public Layout startLayout(){
		...
		vl.setMargin(true);
		vl.setSpacing(true);
		Label tps = new Label("Temps de scan estimé : "+UniscanStatic.TPS);
		Label desc = new Label(UniscanStatic.DESCRITPION,ContentMode.HTML);
